        ...
        <div style={{ padding:24 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
            <div>
              ...
              <h2 style={{ fontFamily:"Georgia,serif", fontSize:17, fontWeight:700, color:"#1e1033", margin:0 }}>{doc.emoji} {doc.label}</h2>
          ...
          <div onClick={() => !file && ref.current?.click()}
            ...
            {!file ? (
              ...
              <div style={{ display:"flex", alignItems:"center", gap:12, justifyContent:"center" }}>
                <div style={{ width:44, height:44, borderRadius:10, background:"#ede9fe", display:"flex", alignItems:"center", justifyContent:"center", fontSize:20 }}>{ext==="PDF"?"📄":"🖼️"}</div>
                ...
                <button onClick={e=>{e.stopPropagation();setFile(null);}} style={{ marginLeft:"auto", background:"#fee2e2", border:"none", borderRadius:8, padding:"4px 8px", fontSize:11, color:"#dc2626", cursor:"pointer" }}>✕ Quitar</button>
              </div>
            )}
          </div>
            ...
            {[
              ...
              <button key={f.id} onClick={()=>setFiltro(f.id)}
                style={{ padding:"7px 14px", borderRadius:99, border:"none", cursor:"pointer", fontSize:12, fontWeight:600, transition:"all .12s", fontFamily:"inherit",
                  background:filtro===f.id?"#7c3aed":"#fff", color:filtro===f.id?"#fff":"#6b7280",
            ...
            {filtrados.map((r, i) => {
              ...
              return (
                <div key={r.tipo} className="doc-row"
                  style={{ display:"flex", alignItems:"center", gap:14, padding:"16px 20px", borderBottom:i<filtrados.length-1?"1px solid #f5eef8":"none", background:"#fff" }}>
                  <div style={{ width:46, height:46, borderRadius:14, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, background:subido?(estadoCfg?.bg||"#fef3c7"):"#f3f4f6" }}>
                    {r.emoji}
                  </div>
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <p style={{ fontSize:11, color:"#9a7080", margin:0 }}>Formatos: {r.formatos}</p>
                    {subido && <p style={{ fontSize:11, color:"#6b7280", margin:"2px 0 0" }}>📎 {subido.nombre} · {subido.tamano_kb} KB</p>}
                  ...
                  {subido ? (
                    ...
                    <div style={{ fontSize:11, fontWeight:600, color:"#9ca3af", background:"#f3f4f6", padding:"5px 12px", borderRadius:99, flexShrink:0 }}>Sin subir</div>
                  )}
