...
import { Send, Bell, Calendar, ArrowRight, CheckCheck, Check } from "lucide-react";
...
export default function MensajesPage() {
  ...
  useEffect(() => {
    cargar();
    // Polling cada 5s para simular tiempo real
  ...
  const enviar = async () => {
    ...
    setMensajes(prev => [...prev, {
      id: Date.now(), remitente: "usuario", contenido: txt,
      leido: 0, created_at: new Date().toISOString(),
    }]);

    await fetch("/api/dashboard/mensajes", {
      method: "POST",
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#faf5f6", fontFamily:"system-ui,-apple-system,sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      {/* TOP HEADER */}
      <div style={{ background:"#fff", borderBottom:"1px solid #ece8f0", padding:"14px 28px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, flexShrink:0 }}>
        <div>
          <h1 style={{ fontFamily:"Georgia,serif", fontSize:22, fontWeight:700, color:"#1e1033", margin:0 }}>¡Hola, {user?.nombre}! 👋</h1>
          <p style={{ fontSize:13, color:"#9a7080", margin:"2px 0 0" }}>Estamos aquí para ayudarte en cada paso. 💜</p>
        </div>
      ...
      <div style={{ flex:1, maxWidth:1400, width:"100%", margin:"0 auto", padding:"20px 24px 24px", display:"flex", gap:20, minHeight:0 }}>

        {/* CHAT */}
        <div style={{ flex:1, display:"flex", flexDirection:"column", background:"#fff", borderRadius:20, border:"1px solid #ece4f0", overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,.04)", minHeight:0 }}>

          {/* Chat header */}
          <div style={{ padding:"14px 20px", borderBottom:"1px solid #f0e8f8", display:"flex", alignItems:"center", gap:12, flexShrink:0 }}>
            <div style={{ width:42, height:42, borderRadius:"50%", background:"linear-gradient(135deg,#7c3aed,#a0435f)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              <span style={{ fontSize:18 }}>✈️</span>
            </div>
            <div>
              <p style={{ fontSize:14, fontWeight:700, color:"#1e1033", margin:0 }}>Destino Au Pair</p>
              <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                <div style={{ width:7, height:7, borderRadius:"50%", background:"#10b981" }}/>
                <p style={{ fontSize:11, color:"#10b981", margin:0, fontWeight:600 }}>En línea</p>
              </div>
            </div>
          </div>

          {/* Mensajes */}
          <div style={{ flex:1, overflowY:"auto", padding:"20px 20px 8px", display:"flex", flexDirection:"column", gap:6 }}>
            {mensajes.length === 0 ? (
              <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"40px 20px" }}>
                <div style={{ fontSize:48, marginBottom:12 }}>💬</div>
                <p style={{ fontSize:15, fontWeight:600, color:"#1e1033", margin:"0 0 6px" }}>¡Bienvenida!</p>
                <p style={{ fontSize:13, color:"#9a7080", margin:0, maxWidth:280, lineHeight:1.5 }}>
                  Este es tu canal directo con el equipo de Destino Au Pair. Escríbenos cualquier duda 💕
                </p>
              </div>
            ) : (
              <>
                {/* Fecha separador al inicio */}
                ...
                {mensajes.map((m, i) => {
                  ...
                  const esHoy     = new Date(m.created_at).toDateString() === new Date().toDateString();
                  ...
                  return (
                    <div key={m.id}>
                      ...
                      <div style={{ display:"flex", justifyContent:esUsuaria?"flex-end":"flex-start", marginTop:mismoRem?2:8, animation:"fadeUp .2s ease" }}>
                        {/* Avatar admin */}
                        {!esUsuaria && !mismoRem && (
                          <div style={{ width:28, height:28, borderRadius:"50%", background:"linear-gradient(135deg,#7c3aed,#a0435f)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, marginRight:6, flexShrink:0, alignSelf:"flex-end", marginBottom:2 }}>
                            ✈️
                          </div>
                        )}
                        {!esUsuaria && mismoRem && <div style={{ width:34, flexShrink:0 }}/>}

                        <div style={{ maxWidth:"72%", display:"flex", flexDirection:"column", alignItems:esUsuaria?"flex-end":"flex-start" }}>
                          <div style={{
                            padding:"10px 14px",
                            borderRadius:esUsuaria ? "18px 18px 4px 18px" : "18px 18px 18px 4px",
                            background:esUsuaria ? "linear-gradient(135deg,#7c3aed,#a0435f)" : "#f3f4f6",
                            color:esUsuaria ? "#fff" : "#1e1033",
                          ...
                          <div style={{ display:"flex", alignItems:"center", gap:4, marginTop:2 }}>
                            ...
                            {esUsuaria && (
                              m.leido
                                ...
                                : <Check size={12} style={{ color:"#9ca3af" }}/>
                            )}
                          </div>
                        </div>
                      </div>
          ...
          {/* Input */}
          <div style={{ padding:"12px 16px", borderTop:"1px solid #f0e8f8", display:"flex", gap:10, alignItems:"flex-end", flexShrink:0 }}>
            <textarea
              ref={inputRef}
              value={texto}
              ...
              style={{
                ...
                padding:"10px 14px", fontSize:13, color:"#1e1033",
            ...
            <button onClick={enviar} disabled={!texto.trim() || enviando}
              style={{
                width:42, height:42, borderRadius:14, border:"none", cursor:(!texto.trim()||enviando)?"not-allowed":"pointer",
                background:(!texto.trim()||enviando)?"#e5e7eb":"linear-gradient(135deg,#7c3aed,#a0435f)",
              ...
              {enviando
                ...
                : <Send size={17} style={{ color:(!texto.trim()||enviando)?"#9ca3af":"#fff" }}/>
              }
            </button>
          </div>
        </div>
        ...
        <aside style={{ width:260, flexShrink:0, display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ background:"linear-gradient(135deg,#7c3aed15,#ede9fe)", border:"1px solid #c4b5fd", borderRadius:20, padding:18 }}>
            ...
            <p style={{ fontSize:12, color:"#6b4a9a", margin:0, lineHeight:1.5 }}>
              Escríbenos cualquier duda sobre tu proceso. Estamos aquí para acompañarte en cada paso. 💕
            </p>
          </div>
          ...
          <div style={{ background:"#fff", borderRadius:20, border:"1px solid #ece4f0", padding:18 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {[
                ...
                <button key={i} onClick={() => setTexto(t)} style={{ textAlign:"left", background:"#faf5ff", border:"1px solid #ede9fe", borderRadius:10, padding:"8px 12px", fontSize:12, color:"#5b21b6", cursor:"pointer", fontFamily:"inherit", transition:"all .12s" }}>
                  {t}
                </button>
              ))}
            </div>
