...
function ResetPasswordForm() {
  ...
  /* ── Sin token ── */
  if (!token) return (
    <div className="bg-white rounded-3xl border border-[#F5E1E7] shadow-xl shadow-[#a0435f]/8 px-8 py-10 text-center">
      <div className="text-5xl mb-4">😕</div>
      <h2 className="font-serif font-bold text-[22px] text-[#3A2530] mb-3">Enlace inválido</h2>
      <p className="text-[13px] text-[#9C8790] mb-6 leading-relaxed">
        Este enlace no es válido o ya expiró. Solicita uno nuevo desde la página de inicio de sesión.
      </p>
      <Link href="/forgot-password"
        className="block w-full bg-[#a0435f] text-white font-semibold text-[14px] py-3.5 rounded-2xl text-center no-underline hover:bg-[#7D2F47] transition">
  ...
  if (exito) return (
    <div className="bg-white rounded-3xl border border-[#F5E1E7] shadow-xl shadow-[#a0435f]/8 px-8 py-10 text-center">
      ...
      <h2 className="font-serif font-bold text-[24px] text-[#3A2530] mb-3">¡Contraseña actualizada!</h2>
      <p className="text-[13px] text-[#9C8790] mb-6 leading-relaxed">
      ...
      <div className="w-full bg-[#F5E1E7] rounded-full h-1.5 overflow-hidden mb-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#C9A9B4]" strokeWidth={1.5}/>
            <input
              type={showP?"text":"password"} required
              placeholder="••••••••"
              value={password} onChange={e => setPassword(e.target.value)}
              className="w-full border border-[#F5E1E7] rounded-2xl px-4 pl-11 pr-11 py-3.5 text-[14px] text-[#3A2530] bg-white placeholder:text-[#C9A9B4] focus:outline-none focus:ring-2 focus:ring-[#C77D93]/30 focus:border-[#C77D93] transition"/>
            <button type="button" onClick={() => setShowP(s => !s)}
              className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#C9A9B4] hover:text-[#a0435f] transition">
            ...
            <div className="mt-3 space-y-1.5">
              {reqs.map(r => (
                <div key={r.label} className="flex items-center gap-2">
                  <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${r.ok?"bg-green-500":"bg-[#F5E1E7]"}`}>
                    {r.ok && <CheckCircle2 size={10} className="text-white"/>}
                  </div>
                  <span className={`text-[11px] ${r.ok?"text-green-600 font-semibold":"text-[#9C8790]"}`}>{r.label}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full border rounded-2xl px-4 pl-11 pr-11 py-3.5 text-[14px] text-[#3A2530] bg-white placeholder:text-[#C9A9B4] focus:outline-none focus:ring-2 transition
                ${confirmar && !coinciden
                  ? "border-red-300 focus:ring-red-200/50 focus:border-red-400"
                  : coinciden
                  ? "border-green-300 focus:ring-green-200/50 focus:border-green-400"
                  : "border-[#F5E1E7] focus:ring-[#C77D93]/30 focus:border-[#C77D93]"}`}/>
        ...
        <button type="submit" disabled={loading || !passwordOk || !coinciden}
          className="w-full bg-[#a0435f] hover:bg-[#7D2F47] disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold text-[15px] py-4 rounded-2xl shadow-lg shadow-[#a0435f]/20 transition">
          {loading ? "Actualizando..." : "Restablecer contraseña"}
        </button>
      </form>

      <div className="mt-6 pt-5 border-t border-[#F5E1E7] text-center">
        <Link href="/login" className="text-[13px] text-[#9C8790] hover:text-[#a0435f] transition">
  ...
  return (
    <div className="min-h-screen bg-[#FBF4F6] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <Suspense fallback={
          <div className="bg-white rounded-3xl border border-[#F5E1E7] p-10 flex justify-center">
