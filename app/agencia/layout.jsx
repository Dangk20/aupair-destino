...
export default function AgenciaLayout({ children }) {
  ...
  const Sidebar = () => (
    <aside style={{
      width:200, flexShrink:0, background:"#4A2A38",
      ...
      <div style={{ padding:"20px 16px 14px", borderBottom:"1px solid rgba(255,255,255,.08)" }}>
        <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:8 }}>
          <div style={{ width:36,height:36,borderRadius:10,background:"#A0435F",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18 }}>✈️</div>
          <div>
            <p style={{ fontSize:8,fontWeight:700,color:"#C77D93",margin:0,textTransform:"uppercase",letterSpacing:1 }}>Portal Agencia</p>
        ...
        <div style={{ background:"rgba(124,92,196,.3)",borderRadius:8,padding:"6px 10px",border:"1px solid rgba(124,92,196,.4)" }}>
          ...
          <p style={{ fontSize:12,fontWeight:600,color:"#C77D93",margin:0 }}>Aliada</p>
        </div>
      </div>
        ...
        {NAV.map(item=>{
          ...
          return (
            <Link key={item.href} href={item.href}
              style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,textDecoration:"none",fontSize:13,fontWeight:active?600:400,color:active?"#fff":"rgba(255,255,255,.55)",background:active?"#A0435F":"transparent",transition:"all .15s" }}>
      ...
      <div style={{ margin:"0 10px 10px",background:"rgba(124,92,196,.15)",borderRadius:12,padding:"12px",border:"1px solid rgba(124,92,196,.2)" }}>
        <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:6 }}>
          <div style={{ width:28,height:28,borderRadius:"50%",background:"rgba(124,92,196,.3)",display:"flex",alignItems:"center",justifyContent:"center" }}>
            <HelpCircleIcon size={14} style={{ color:"#C77D93" }}/>
          </div>
        ...
        <a href="mailto:hola@destino-aupair.com"
          style={{ display:"block",textAlign:"center",background:"#A0435F",color:"#fff",fontSize:11,fontWeight:600,padding:"7px",borderRadius:8,textDecoration:"none" }}>
          Contactar soporte
        </a>
      </div>
      ...
      <div style={{ padding:"12px 14px",borderTop:"1px solid rgba(255,255,255,.08)" }}>
        <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:10 }}>
          <div style={{ width:32,height:32,borderRadius:"50%",background:"#A0435F",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:"#fff",flexShrink:0 }}>
            {user?.nombre?.[0]||"A"}
          </div>
  ...
  return (
    <div style={{ display:"flex",minHeight:"100vh",background:"#FBF4F6",fontFamily:"system-ui,-apple-system,sans-serif" }}>
      ...
      <div style={{ flex:1,minWidth:0,display:"flex",flexDirection:"column" }}>
        {isMobile && (
          <div style={{ background:"#fff",borderBottom:"1px solid #E5E7EB",padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:30 }}>
            <button onClick={()=>setOpen(o=>!o)} style={{ background:"none",border:"none",cursor:"pointer",color:"#4A2A38" }}>
              {open?<XIcon size={20}/>:<MenuIcon size={20}/>}
            </button>
            <p style={{ fontFamily:"Georgia,serif",fontSize:15,fontWeight:700,color:"#4A2A38",margin:0 }}>Portal Agencia</p>
            <div style={{ width:20 }}/>
          </div>
        )}
