    ...
    <div style={{ minHeight:"100vh",background:"#f5f3ff",fontFamily:"system-ui,-apple-system,sans-serif" }}>
      ...
      <div style={{ background:"#fff",borderBottom:"1px solid #e9e3f8",padding:isMobile?"14px 16px":"20px 28px" }}>
        <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12 }}>
          <div>
            <h1 style={{ fontFamily:"Georgia,serif",fontSize:isMobile?20:24,fontWeight:700,color:"#1e1033",margin:0 }}>Documentos</h1>
            <p style={{ fontSize:13,color:"#9a7080",margin:"4px 0 0" }}>Revisa los documentos subidos por cada candidata.</p>
          </div>
        </div>
      </div>
        ...
        <div style={{ background:"#fff",borderRadius:16,border:"1px solid #e9e3f8",padding:"12px 16px",marginBottom:16,boxShadow:"0 1px 4px rgba(0,0,0,.04)" }}>
          <div style={{ position:"relative" }}>
            ...
            <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="Buscar candidata..."
              style={{ width:"100%",paddingLeft:30,height:36,border:"1.5px solid #e9e3f8",borderRadius:8,fontSize:12,color:"#374151",outline:"none",fontFamily:"inherit",boxSizing:"border-box" }}/>
          </div>
        </div>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
          {filtradas.length===0 ? (
            <div style={{ background:"#fff",borderRadius:16,border:"1px solid #e9e3f8",padding:"48px 20px",textAlign:"center" }}>
              ...
              <p style={{ fontSize:14,fontWeight:600,color:"#1e1033",margin:0 }}>No hay candidatas con perfil completado</p>
            </div>
          ) : filtradas.map(c=>(
            <div key={c.id} style={{ background:"#fff",borderRadius:16,border:"1px solid #e9e3f8",overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,.04)" }}>
              {/* Header candidata */}
              <div style={{ padding:"14px 20px",borderBottom:"1px solid #f0edff",display:"flex",alignItems:"center",gap:12 }}>
                <div style={{ width:36,height:36,borderRadius:"50%",background:"#ede9f8",overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
                  {c.foto_url?<img src={c.foto_url} alt="" style={{ width:"100%",height:"100%",objectFit:"cover" }}/>:<span style={{ fontSize:13,fontWeight:700,color:"#7c5cc4" }}>{c.nombre?.[0]}</span>}
                </div>
                <div>
                  <p style={{ fontSize:13,fontWeight:700,color:"#1e1033",margin:0 }}>{c.nombre} {c.apellido}</p>
                  <p style={{ fontSize:11,color:"#9a7080",margin:0 }}>{PAISES_EMOJI[c.pais]||""} {c.pais||"—"} · ID: DA-{String(c.id).padStart(4,"0")}</p>
                </div>
              </div>
              ...
              <div style={{ padding:"14px 20px",display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:10 }}>
                {Object.entries(DOCS_LABELS).map(([key,label])=>{
                  const tiene = !!c[key];
                  return (
                    <div key={key} style={{ background:tiene?"#f0fdf4":"#fafafa",borderRadius:10,padding:"10px 12px",border:`1px solid ${tiene?"#bbf7d0":"#e9e3f8"}`,display:"flex",alignItems:"center",gap:8 }}>
                      <FileTextIcon size={14} style={{ color:tiene?"#5a8a3a":"#9a7080",flexShrink:0 }}/>
                      <div style={{ minWidth:0 }}>
                        <p style={{ fontSize:11,fontWeight:600,color:tiene?"#1e1033":"#9a7080",margin:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{label}</p>
                        <p style={{ fontSize:10,color:tiene?"#5a8a3a":"#c0a0a8",margin:0,fontWeight:600 }}>{tiene?"✓ Disponible":"Sin subir"}</p>
                      </div>
                    </div>
                  );
                })}
              ...
              {c.foto_url && (
                <div style={{ padding:"0 20px 14px",display:"flex",alignItems:"center",gap:10 }}>
                  ...
                  <div>
                    <p style={{ fontSize:12,fontWeight:600,color:"#1e1033",margin:0 }}>Foto de perfil</p>
                    <a href={c.foto_url} target="_blank" rel="noreferrer" style={{ display:"inline-flex",alignItems:"center",gap:4,fontSize:11,color:"#7c5cc4",fontWeight:600,marginTop:4,textDecoration:"none" }}>
