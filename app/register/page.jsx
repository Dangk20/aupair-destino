...
/* ══ Contenido separado para poder usar useSearchParams ══ */
    ...
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div className="lg:w-[45%] relative flex flex-col items-center justify-center
        ...
        <div className="relative z-10 text-center w-full max-w-xs mx-auto">
          ...
          <h2 className="font-serif font-bold text-white text-[32px] lg:text-[38px] leading-tight mb-1 drop-shadow-lg">
            Tu Destino
          </h2>
          <h2 className="font-serif font-bold italic text-white text-[32px] lg:text-[38px] leading-tight mb-4 drop-shadow-lg">
            empieza aquí.
          </h2>
          <div className="flex justify-center mb-4">
            <span className="text-white/50 text-[20px]">♡</span>
          </div>
          <p className="text-white/85 text-[13px] xl:text-[14px] leading-relaxed max-w-[240px] mx-auto mb-8 font-medium drop-shadow-sm">
            Regístrate gratis y accede a la sesión<br/>
            de bienvenida. Da el primer paso<br/>
            sin compromiso.
          </p>

          <div className="bg-white/12 border border-white/20 rounded-2xl p-4 text-left backdrop-blur-sm mb-5 space-y-4">
            {steps.map((s) => (
              <div key={s.num} className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-full bg-white/20 border border-white/25 flex items-center justify-center shrink-0 text-white">
                  {s.icon}
                </div>
                <div>
              ...
              {badges.map((b, i) => {
                ...
                return (
                  <div key={i} className="flex flex-col items-center gap-1.5">
                    ...
                    <p className="text-white/70 text-[9px] text-center leading-snug whitespace-pre-line">{b.label}</p>
                  </div>
                );
              })}
            </div>
      ...
      <div className="flex-1 bg-[#f8f5ff] flex items-center justify-center
        ...
        <div className="w-full max-w-md xl:max-w-lg">
          <div className="bg-white rounded-3xl border border-[#e0d4f5] shadow-xl shadow-[#7c5cc4]/10 px-7 xl:px-9 py-8 xl:py-10">
            ...
            <h1 className="font-serif font-bold text-center text-[28px] xl:text-[32px] text-[#1a0a3d] mb-1">
              Crea tu{" "}<span className="italic text-[#7c5cc4]">cuenta</span>
            </h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] xl:text-[11px] font-bold tracking-[2px] uppercase text-[#1a0a3d] mb-1.5">Nombre</label>
                  <div className="relative">
                    <User size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#b0a0d0]" strokeWidth={1.5}/>
                    <input name="nombre" type="text" placeholder="María" value={form.nombre} onChange={handleChange} required className={inputBase}/>
                  </div>
                </div>
                <div>
                  <label className="block text-[10px] xl:text-[11px] font-bold tracking-[2px] uppercase text-[#1a0a3d] mb-1.5">Apellido</label>
                  <div className="relative">
                    <User size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#b0a0d0]" strokeWidth={1.5}/>
                    <input name="apellido" type="text" placeholder="García" value={form.apellido} onChange={handleChange} required className={inputBase}/>
                  </div>
                </div>
              </div>

              <div>
                <label className="block text-[10px] xl:text-[11px] font-bold tracking-[2px] uppercase text-[#1a0a3d] mb-1.5">Correo electrónico</label>
                <div className="relative">
                  <Mail size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#b0a0d0]" strokeWidth={1.5}/>
                  <input name="email" type="email" placeholder="tu@correo.com" value={form.email} onChange={handleChange} required className={inputBase}/>
                </div>
              </div>

              <div>
                <label className="block text-[10px] xl:text-[11px] font-bold tracking-[2px] uppercase text-[#1a0a3d] mb-1.5">Contraseña</label>
                <div className="relative">
                  <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#b0a0d0]" strokeWidth={1.5}/>
                  <input name="password" type={showPwd ? "text" : "password"} placeholder="Mínimo 8 caracteres"
                         value={form.password} onChange={handleChange} required className={`${inputBase} pr-11`}/>
              ...
              <div>
                <label className="block text-[10px] xl:text-[11px] font-bold tracking-[2px] uppercase text-[#1a0a3d] mb-1.5">Confirmar contraseña</label>
                <div className="relative">
                  <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#b0a0d0]" strokeWidth={1.5}/>
                  <input name="confirm" type={showPwd2 ? "text" : "password"} placeholder="Repite tu contraseña"
                         value={form.confirm} onChange={handleChange} required className={`${inputBase} pr-11`}/>
              ...
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-[12px] xl:text-[13px] text-[#5a4080] leading-relaxed">
                  Acepto los{" "}
                  <Link href="/terminos" className="text-[#7c5cc4] font-semibold hover:underline">Términos de uso</Link>{" "}
                  y la{" "}
                  <Link href="/privacidad" className="text-[#7c5cc4] font-semibold hover:underline">Política de privacidad</Link>
                </span>
              </label>
              ...
              <button type="submit" disabled={loading}
                className="w-full bg-[#7c5cc4] hover:bg-[#6a4ab0] disabled:opacity-60
                           ...
                           py-4 rounded-2xl shadow-lg shadow-[#7c5cc4]/30 transition-all duration-200">
                {loading ? "Creando cuenta..." : "Comenzar gratis →"}
              </button>

              <p className="text-center text-[13px] text-[#7060a0]">
                ¿Ya tienes cuenta?{" "}
                <Link href="/login" className="text-[#7c5cc4] font-bold hover:underline">Iniciar sesión</Link>
              </p>
            </form>
            ...
            <div className="mt-6 pt-5 border-t border-[#ede9f8] text-center">
              <div className="flex items-center justify-center gap-2 mb-1">
                ...
                <p className="text-[12px] font-semibold text-[#1a0a3d]">Tus datos están protegidos</p>
              </div>
...
/* ══ Página exportada — envuelve en Suspense ══ */
export default function RegisterPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-[#f8f5ff] flex items-center justify-center">
        <div className="w-10 h-10 border-2 border-[#7c5cc4] border-t-transparent rounded-full animate-spin"/>
      </div>
    }>
      <RegisterContent />
    </Suspense>
