...
export default function VentasAdminPage() {
  ...
  return (
    <div className="p-5 md:p-8 max-w-5xl mx-auto">
      ...
      <p className="text-[13px] text-[#9a7080] mb-5">Intenciones de pago de las candidatas. Confirma cuando recibas el pago.</p>

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Stat icon={ClockIcon}       label="Pendientes"  value={stats.pendientes || 0}  color="#c9973a" />
        <Stat icon={CheckCircleIcon} label="Confirmadas"  value={stats.confirmadas || 0} color="#5a8a3a" />
        <Stat icon={CreditCardIcon}  label="Total ventas" value={stats.total || 0}       color="#a0435f" />
        <Stat icon={TagIcon}         label="Ingresos"     value={money(stats.ingresos_confirmados)} color="#6b4f9e" />
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-[#f0dde2] p-10 text-center">
          <p className="text-[13px] text-[#9a7080]">No hay ventas {tab !== "todos" ? `en estado "${tab}"` : ""}.</p>
        </div>
      ) : (
        <div className="space-y-2.5">
          {lista.map((v) => (
            <div key={v.id} className="bg-white rounded-2xl border border-[#f0dde2] p-4 flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[14px] font-semibold text-[#3a1420] truncate">{v.candidata}</p>
                <p className="text-[11px] text-[#9a7080] truncate">{v.email}</p>
              ...
              <div className="flex items-center gap-3 shrink-0">
                {v.codigo ? (
                  ...
                  <span className="text-[10px] text-[#b89aa4] bg-[#f7f0f2] px-2.5 py-1 rounded-full">Directo</span>
                )}
                <span className="text-[15px] font-bold text-[#3a1420] w-[92px] text-right">{money(v.monto)}</span>
                {v.estado === "pendiente" ? (
                  <div className="flex items-center gap-2">
                    <button onClick={() => setModal({ venta: v, accion: "confirmar" })} title="Confirmar pago"
                      className="w-9 h-9 rounded-xl bg-[#5a8a3a] hover:bg-[#4a7530] text-white flex items-center justify-center transition">
                      <Check size={17} />
                    </button>
        ...
        return (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4" style={{ background: "rgba(58,20,32,.45)", backdropFilter: "blur(3px)" }}
            ...
            <div className="bg-white rounded-3xl w-full max-w-[400px] p-6 shadow-2xl">
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4 ${esConfirmar ? "bg-[#eaf3e4] text-[#5a8a3a]" : "bg-[#fdecec] text-[#c0392b]"}`}>
                {esConfirmar ? <Check size={26} /> : <Ban size={24} />}
              </div>
              <h3 className="text-[17px] font-bold text-[#3a1420] text-center mb-1">
                {esConfirmar ? "Confirmar pago" : "Anular solicitud"}
              </h3>
              <p className="text-[13px] text-[#9a7080] text-center leading-relaxed mb-1">
                {esConfirmar
                  ? <>Vas a confirmar el pago de <b className="text-[#3a1420]">{v.candidata}</b> por <b className="text-[#3a1420]">{money(v.monto)}</b>.</>
                  : <>Vas a anular la solicitud de <b className="text-[#3a1420]">{v.candidata}</b> por <b className="text-[#3a1420]">{money(v.monto)}</b>.</>}
              </p>
              <p className="text-[12px] text-[#b89aa4] text-center mb-5">
                {esConfirmar
              ...
              <div className="flex gap-3">
                <button onClick={() => !procesando && setModal(null)} disabled={procesando}
                  className="flex-1 py-3 rounded-xl border border-[#f0dde2] text-[#9a7080] text-[13px] font-semibold hover:bg-[#faf5f6] transition">
                  Cancelar
                </button>
                <button onClick={ejecutar} disabled={procesando}
                  className={`flex-1 py-3 rounded-xl text-white text-[13px] font-semibold transition disabled:opacity-60 ${esConfirmar ? "bg-[#5a8a3a] hover:bg-[#4a7530]" : "bg-[#c0392b] hover:bg-[#a93226]"}`}>
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-[#3a1420] text-white text-[13px] font-medium px-5 py-3 rounded-xl shadow-lg z-50">
          {toast}
        </div>
      )}
    </div>
