...
export async function confirmarVenta(ventaId) {
  ...
  // 3) registrar el uso del código: contador de usos_max + stats del módulo admin.
  //    Idempotente: sólo si esta candidata no tenía ya un uso registrado del código.
  let comision = null;
  if (venta.codigo_promo_id) {
    const [[yaUso]] = await dbAupair.query(
      "SELECT id FROM codigos_promo_usos WHERE codigo_id = ? AND usuario_id = ?",
      [venta.codigo_promo_id, venta.usuario_id]
    );
    if (!yaUso) {
      await dbAupair.query(
        "INSERT INTO codigos_promo_usos (codigo_id, usuario_id, monto_pagado) VALUES (?, ?, ?)",
        [venta.codigo_promo_id, venta.usuario_id, Number(venta.monto)]
      );
      await dbAupair.query(
    ...
    // 4) comisión, sólo si el código tiene asociada y porcentaje > 0
